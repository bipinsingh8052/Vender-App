...
const TaxPage = () => {
  ...
  const reload = async () => {
    setload(true);
    let api = `${BASE_URL}/vendor/taxrule?page=1&limit=10`;
    try {
  ...
  const handleDelete = async (id) => {
    let api = `${BASE_URL}/vendor/taxrule/${id}`;
    try {
  ...
  return (
    <>
        ...
        <View style={{
          ...
          <Text style={{ color: 'white', fontSize: 18 }}>Tax</Text>
          <TouchableOpacity
            style={{
            ...
            onPress={() => { Router.push('/(vender)/(components)/TaxForm') }}
          >
                  ...
                  <DataTable.Row key={rowItem.id}>
                    {options
                      ...
                      .map((opt) => (
                        ...
                        >
                          {opt.index === 0 && <Text>{rowItem.country}</Text>}
                          {opt.index === 1 && <Text>{rowItem.state}</Text>}
                          {opt.index === 2 && <Text>{rowItem.type}</Text>}
                          {opt.index === 3 && <Text>{rowItem.rate}</Text>}
                          {opt.index === 4 && (
                            <View style={{ flexDirection: 'row', gap:5 }}>
                              <TouchableOpacity style={{justifyContent:"center", alignContent:"center"}} onPress={()=>{
                                Router.push({
                                  pathname:'/(vender)/(components)/TaxView',params:{id:rowItem.id}})
                                }}>  
                                <Entypo name='eye' size={20} color={"green"}/>
                              </TouchableOpacity>
                              <TouchableOpacity onPress={() => {
                                Router.push({
                                  pathname: '/(vender)/(components)/TaxForm',
                                  params: {
                                    id: rowItem.id,
