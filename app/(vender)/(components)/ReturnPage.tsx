...
const ReturnPage = () => {
  ...
  const reload = async () => {
    setload(true);
    let api = `${BASE_URL}/vendor/return`;
    try {
  ...
  const handleDelete = async (id) => {
    let api = `${BASE_URL}/vendor/return/${id}`;
    try {
        ...
        <View style={{flex:1, alignItems:"center", justifyContent:"center"}}>
         ...
         <ScrollView style={{ padding: 10, backgroundColor: '#f4f4f4' }}>
                          ...
                          {opt.index === 4 && (
                            <View style={{ flexDirection: 'row', gap:5 }}>
                              <TouchableOpacity style={{justifyContent:"center", alignContent:"center"}} onPress={()=>{
                                Router.push({
                                  pathname:'/(vender)/(components)/ReturnView',params:{id:rowItem.id}})
                                }}>  
                                <Entypo name='eye' size={20} color={"green"}/>
                              </TouchableOpacity>
                              <TouchableOpacity onPress={() => {
                                Router.push({
                                  pathname: '/(vender)/(components)/ReturnForm',
                                  params: {
                                    id: rowItem.id,
                                    // country: rowItem.c,
          ...
          <TouchableOpacity style={{
              ...
              disabled onPress={()=>{Router.push('/(vender)/(components)/ReturnForm')}}>
