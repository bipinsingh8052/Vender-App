...
const RefundPage = () => {
  ...
  const [options, setOptions] = useState([
    { label: 'Refund Request Id', checked: true },
    { label: 'Reason', checked: true },
    { label: 'Order Price', checked: true },
  ...
  const reload = async () => {
    setload(true);
    let api = `${BASE_URL}/vendor/refund`;
    try {
  ...
  const handleDelete = async (id) => {
    let api = `${BASE_URL}/vendor/refund/${id}`;
    try {
  ...
  return (
    <>
        ...
        <View style={{
          ...
          <Text style={{ color: 'white', fontSize: 18 }}>Refund's</Text>
          <TouchableOpacity
            style={{
            ...
            onPress={() => { Router.push('/(vender)/(components)/RefundForm') }}
          >
                    ...
                    {options
                      ...
                      .map((opt) => (
                        ...
                        >
                          ...
                          {opt.index === 1 && <Text>{rowItem.reason}</Text>}
                          {opt.index === 2 && <Text>{rowItem.order.netTotal}</Text>}
                          {opt.index === 3 && <Text>{rowItem.status}</Text>}
                          {opt.index === 4 && (
                            <View style={{ flexDirection: 'row', gap:5 }}>
                              <TouchableOpacity style={{justifyContent:"center", alignContent:"center"}} onPress={()=>{
                                Router.push({
                                  pathname:'/(vender)/(components)/RefundView',params:{id:rowItem.id}})
                                }}>  
                                <Entypo name='eye' size={20} color={"green"}/>
                              </TouchableOpacity>
                              <TouchableOpacity onPress={() => {
                                Router.push({
                                  pathname: '/(vender)/(components)/RefundForm',
                                  params: {
                                    id: rowItem.id,
                                    // country: rowItem.c,
