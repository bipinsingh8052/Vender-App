...
export default function RefundForm() {
  const { id,  status,reason } = useLocalSearchParams(); // Destructure parameters directly
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        let res=await axios.patch(`${BASE_URL}/vendor/refund/${id}`, payload);
  ...
  ToastAndroid.CENTER // or ToastAndroid.TOP / BOTTOM
        ...
        let res=await axios.post(`${BASE_URL}/vendor/refund`, payload);
       ToastAndroid.showWithGravity(
        ...
        <View style={{ marginTop: 32, alignItems: 'center' }}>
          ...
          >
            <Text style={{ color: '#fff', fontWeight: 'bold' }}>
              {isEdit ? "Update Refund" : "Add Refund"}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
