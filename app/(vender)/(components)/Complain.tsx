...
const ComplainPage = () => {
  ...
  const [options, setOptions] = useState([
    { label: 'Complaint Id', checked: true },
    { label: 'Complaint', checked: true },
  ...
  const reload = async () => {
    setload(true);
    let api = `${BASE_URL}/vendor/complaint?page=1&limit=10`;
    try {
  ...
  const handleDelete = async (id) => {
    let api = `${BASE_URL}/vendor/complaint/${id}`;
    try {
  ...
  return (
    <>
        ...
        <View style={{
          ...
          <Text style={{ color: 'white', fontSize: 18 }}>Complain's</Text>
          <TouchableOpacity
            style={{
            ...
            onPress={() => { Router.push('/(vender)/(components)/ComplainForm') }}
          >
                    ...
                    {options
                      ...
                      .map((opt) => (
                        ...
                        >
                          ...
                          {opt.index === 1 && <Text>{rowItem.complaint}</Text>}
                          {opt.index === 2 && <Text>{rowItem.status}</Text>}
                          {opt.index === 3 && (
                            <View style={{ flexDirection: 'row', gap:5 }}>
                              <TouchableOpacity style={{justifyContent:"center", alignContent:"center"}} onPress={()=>{
                                Router.push({
                                  pathname:'/(vender)/(components)/ComplainView',params:{id:rowItem.id}})
                                }}>  
                                <Entypo name='eye' size={20} color={"green"}/>
                              </TouchableOpacity>
                              <TouchableOpacity onPress={() => {
                                Router.push({
                                  pathname: '/(vender)/(components)/ComplainForm',
                                  params: {
                                    id: rowItem.id,
                                    // country: rowItem.c,
                                    status: rowItem.status,
                                    complaint: rowItem.complaint,
                                    // rate: rowItem.rate,
                                  },
                                });
                              }}>
