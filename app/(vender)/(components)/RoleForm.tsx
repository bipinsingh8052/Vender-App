...
export default function RoleForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        await axios.patch(`${BASE_URL}/vendor/role/${id}`, payload);
        ToastAndroid.showWithGravity('Updated successfully!', ToastAndroid.SHORT, ToastAndroid.CENTER);
      } else {
        await axios.post(`${BASE_URL}/vendor/role`, payload);
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#fff', marginTop: 20 }}>
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 16 }}>
          <View style={{
            ...
            <Text style={{ marginBottom: 6, fontWeight: '600' }}>Enter the Role</Text>
            <TextInput
          ...
          <View style={{ marginTop: 32, alignItems: 'center' }}>
            ...
            >
              <Text style={{ color: '#fff', fontWeight: 'bold' }}>
                {isEdit ? "Update Role's" : "Add Role's"}
              </Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
