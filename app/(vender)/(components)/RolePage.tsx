...
const RolePage = () => {
  ...
  const reload = async () => {
    setLoad(true);
    try {
      const response = await axios.get(`${BASE_URL}/vendor/role`);
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(`${BASE_URL}/vendor/role/${id}`);
      Toast.show({
        type: 'success',
    ...
    <>
      <View style={{ flex: 1, backgroundColor: '#f1f1f1', paddingTop: 10, paddingBottom: 10 }}>
        ...
        <View style={{
          ...
          <Text style={{ color: 'white', fontSize: 18 }}>Role's Page</Text>
          <TouchableOpacity
            style={{
            ...
            onPress={() => Router.push('/(vender)/(components)/RoleForm')}
          >
                    ...
                    {options[1].checked && (
                      <DataTable.Cell style={{ width: 200 }}>
                        <View style={{ flexDirection: 'row', gap: 5 }}>
                          ...
                          <TouchableOpacity disabled onPress={() => Router.push({ pathname: '/(vender)/(components)/RoleForm', params: { id: item.id, name: item.name } })}>
