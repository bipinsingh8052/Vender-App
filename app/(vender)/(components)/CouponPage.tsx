...
const CouponPage = () => {
  ...
  const [options, setOptions] = useState([
     { label: 'Active List', checked: true },
    { label: 'Coupon Code', checked: true },
...
const filteredData = data.filter(item => {
  ...
  return (
    ...
    item.discount?.toLowerCase().includes(searchText) ||
    item.id?.toLowerCase().includes(searchText) ||
    item.isActive?.email?.toLowerCase().includes(searchText) 
  );
});
  ...
  const reload = async () => {
    setload(true);
    let api = `${BASE_URL}/vendor/coupon`;
    try {
  ...
  const handleDelete = async (id) => {
    let api = `${BASE_URL}/vendor/coupon/${id}`;
    try {
  ...
  return (
    <>
        ...
        <View style={{
          ...
          <Text style={{ color: 'white', fontSize: 18 }}>Coupon page</Text>
          <TouchableOpacity
            style={{
            ...
            onPress={() => { Router.push('/(vender)/(components)/CouponForm') }}
          >
                  ...
                  <DataTable.Row key={rowItem.id}>
                    {options
                      ...
                      .map((opt) => (
                        ...
                        >
                          {opt.index === 0 && <Text>{rowItem.isActive ? 'true' : 'false'}</Text>}
                          {opt.index === 1 && <Text>{rowItem.code}</Text>}
                          {opt.index === 2 && <Text>{rowItem.description}</Text>}
                          {opt.index === 3 && <Text style={{textAlign:"center" }} >{rowItem.discount}</Text>}
                          {opt.index === 4 && (
                            <View style={{ flexDirection: 'row', gap:5 }}>
                              ...
                              <TouchableOpacity disabled onPress={() => {
                                Router.push({
                                  pathname: '/(vender)/(components)/CouponForm',
                                  params: {
                                    id: rowItem.id,
                                    code: rowItem.code,
